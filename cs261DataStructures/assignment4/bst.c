...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    while(current != 0)                                 /* while not the end, iterate */
    {
        if(compare(val, current->val) == -1)            /* if val is smaller */
        {
            current = current->left;                    /* go left */
        }
        else if(compare(val, current->val) == 1)        /* if val is larger */
        {
...
TYPE _leftMost(struct Node *cur)
{
	...
	struct Node * tempLefty = cur;

	while (tempLefty->left != 0)                           /* go left until null is found */
    {
        tempLefty = tempLefty->left;
    }
	return (tempLefty->val);
}
...
#if 1
/************************************************************************************************************************
from here to the end of this file are a set of functions for testing the functions of the BST - This is also provided in a
separate file .
***************************************************************************************************************************/
/*
