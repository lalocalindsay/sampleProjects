...
int main(int argc, char* argv[]){
    ...
    /*
    // TESTING MACRO for micro values: EQ(A, B) (fabs(A - B) < 10e-7)
	TYPE test0, test1, test2, test3, test4;
	test0 = EQ(getDynArr(dyn, 0), 3);
	printf("test0: %f\n", test0);

	test1 = EQ(getDynArr(dyn, 1), 4);
	printf("test1: %f\n", test1);

    test2 = EQ(getDynArr(dyn, 2), 10);
	printf("test2: %f\n", test2);

	test3 = EQ(getDynArr(dyn, 3), 5);
	printf("test3: %f\n", test3);

	test4 = EQ(getDynArr(dyn, 4), 6);
	printf("test4: %f\n", test4);
    */
