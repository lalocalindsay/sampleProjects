...
TYPE getMinHeap(DynArr *heap)
{
    ...
    //assert(isEmptyDynArr(heap) != 0);     //crashes
    ...
    task = getDynArr(heap, 0);
    taskL = getDynArr(heap, 1);
    taskR = getDynArr(heap, 2);

    //higher orderAdded is least priority

    //if task has the same priority as its children, look at order added
    if ( (task->priority == taskL->priority) && (task->priority == taskR->priority))
	{
	    if (task->orderAdded <= (taskL->orderAdded && taskR->orderAdded)) //added before or same time as children
            return (heap->data[0]);
        else if (task->orderAdded > taskL->orderAdded ) //it was added after its left child
            return (heap->data[1]); //left has priority
        else if (task->orderAdded > taskR->orderAdded)
            return (heap->data[2]); //right has priority
        // if > and L and R are =, move down
        else if ((taskL->orderAdded == taskR->orderAdded) && ((taskL->orderAdded && taskR->orderAdded) < task->orderAdded))
        {
           //move down
           while ()
...
void _buildHeap(DynArr *heap)
{
    ...
    /* build heap: beginning @ last non-leaf node: (max/2)-1 [if L to R], ending @ root */

    int itr = ((heap->size)/2)-1;
    int i;
    for (i = itr; i >= 0; i--)
    {
